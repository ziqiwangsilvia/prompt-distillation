"""Multi-turn conversation support for training data.

Converts multi-turn message sequences into training samples where each
assistant turn is a separate training target, with all prior turns as context.
Tool calls are formatted in each model's native format.
"""
import json

from data.tool_call_format import format_tool_call, normalize_tool_call
...
def build_multiturn_samples(exercise, llm, teacher_llm=None, tools=None,
                            use_tool_token=False, max_length=0):
    ...
    for turn_idx, msg in enumerate(raw_msgs):
        ...
        student_closed_tokens = llm.tokenize(llm.messages_to_prompt(student_closed_ctx, tools=student_tools))
